...
export const Route = createFileRoute("/visitors")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your OATF visitor dashboard — ticket, QR points, saved exhibitors and personal schedule." },
    ],
  }),
...
function VisitorsPage() {
  return (
    <PageShell
      ...
      title={<>Your fair, <span className="text-gradient-ember">your way.</span></>}
    ...
    >
      ...
      <div className="grid lg:grid-cols-3 gap-5 mb-8">
        <div className="lg:col-span-2 relative overflow-hidden rounded-3xl bg-gradient-dusk p-8 shadow-elegant">
          ...
          <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="text-xs uppercase tracking-[0.3em] text-ember mb-3 inline-flex items-center gap-2">
        ...
        <div className="glass rounded-3xl p-7">
          ...
          <div className="font-display text-6xl font-bold tabular-nums text-gradient-ember mt-4">720</div>
          <div className="text-sm text-muted-foreground mt-2">Rank #3 of 12,480 visitors</div>
          <div className="mt-5 h-2 rounded-full bg-white/10 overflow-hidden">
            <div className="h-full w-[58%] bg-gradient-ember" />
          </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-12">
        {[
          ...
          <Link key={a.title} to={a.to} className="glass rounded-2xl p-5 flex items-center gap-4 hover:-translate-y-0.5 transition-transform">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div>
          <h2 className="font-display text-2xl font-bold mb-4 flex items-center gap-2">
            <Heart className="h-5 w-5 text-ember" /> Saved exhibitors
          </h2>
          <div className="space-y-3">
            {SAVED.map((e) => (
              <Link key={e.id} to="/exhibitors/$id" params={{ id: e.id }} className="glass rounded-2xl p-4 flex items-center gap-4 hover:bg-white/[0.06] transition-colors">
                <div className="h-12 w-12 rounded-xl bg-gradient-ember flex items-center justify-center font-bold text-primary-foreground">{e.name.charAt(0)}</div>
        ...
        <div>
          <h2 className="font-display text-2xl font-bold mb-4 flex items-center gap-2">
            <Trophy className="h-5 w-5 text-ember" /> Leaderboard
          </h2>
          <div className="glass rounded-2xl p-2">
            {LEADERBOARD.map((p, i) => (
              <div
                key={p.name}
                className={`flex items-center gap-4 p-3 rounded-xl ${p.you ? "bg-gradient-ember/10 border border-ember/30" : ""}`}
              >
                <div className={`h-9 w-9 rounded-lg flex items-center justify-center font-bold ${i < 3 ? "bg-gradient-ember text-primary-foreground" : "glass text-muted-foreground"}`}>
                  {i + 1}
                </div>
                <div className={`flex-1 font-display font-semibold ${p.you ? "text-ember" : ""}`}>{p.name}</div>
                <div className="font-mono tabular-nums text-sm">{p.points.toLocaleString()} pts</div>
              </div>
            ))}
          </div>
        </div>
