...
export const Route = createFileRoute("/exhibitors/$id")({
  ...
  notFoundComponent: () => (
    <div className="min-h-screen flex items-center justify-center text-foreground bg-background">
      <div className="text-center">
        ...
        <Link to="/exhibitors" className="text-primary mt-4 inline-block">
          ← Back to directory
        </Link>
      </div>
    </div>
  ...
  errorComponent: ({ error }) => (
    <div className="min-h-screen flex items-center justify-center text-foreground bg-background p-8">
      {error.message}
    </div>
  ),
...
function ExhibitorPage() {
  ...
  const related = EXHIBITORS.filter(
    (e) => e.category === exhibitor.category && e.id !== exhibitor.id,
  ).slice(0, 3);
          ...
          <div className="grid lg:grid-cols-3 gap-12 items-start">
            <div className="lg:col-span-2">
              <div className="flex items-center gap-5 mb-6">
                <div className="h-24 w-24 rounded-2xl bg-gradient-brand flex items-center justify-center font-display text-4xl font-bold text-primary-foreground shadow-elegant">
            ...
            <div className="glass rounded-2xl p-6">
              ...
              <Link
                to="/maps"
                className="mt-4 w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-brand text-primary-foreground font-semibold text-sm shadow-elegant hover:scale-[1.02] transition-transform"
              >
                Show on map
              </Link>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="glass rounded-2xl p-8">
              ...
              <p className="text-muted-foreground leading-relaxed">
                {exhibitor.name} is part of the {exhibitor.category} showcase at OATF 2026. Visit
                their stand at {exhibitor.stand} in {hall?.name} to discover their products,
                services and upcoming launches. Stay tuned for live demonstrations and exclusive
                offers throughout the fair.
              </p>
            </div>
